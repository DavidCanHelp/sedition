...
func (d *TestnetDeployment) createGenesisState(validatorConfigs []*ValidatorConfig) error {
	...
	for i, config := range validatorConfigs {
		...
		genesisValidators[i] = GenesisValidator{
			Address:    config.ID,
			PublicKey:  []byte(config.ValidatorKey),
			Stake:      stake,
			Reputation: 5.0, // Start with neutral reputation
		}

		// Register validator manually (since RegisterValidator doesn't exist)
		validator := &Validator{
			ID:    config.ID,
			Stake: stake,
		}
		if d.consensus.Validators == nil {
			d.consensus.Validators = make(map[string]*Validator)
		}
		d.consensus.Validators[config.ID] = validator
		err := error(nil)
		if err != nil {
	...
	d.genesisState = &GenesisState{
		...
		Timestamp:  d.config.GenesisTime,
...
func (d *TestnetDeployment) createValidator(config *ValidatorConfig) (*TestnetValidator, error) {
	...
	for _, genesisValidator := range d.genesisState.Validators {
		// Skip - validators already registered during genesis setup
		_ = genesisValidator
	}
