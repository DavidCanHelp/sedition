...
func testStorageLayer(t *testing.T, env *TestEnvironment) {
	...
	if retrievedBlock.Height != block.Height {
	...
	err = db.StoreValidator(validator)
	if err != nil {
	...
	if retrievedValidator.Address != validator.Address {
		t.Errorf("Validator address mismatch: expected %s, got %s", 
			validator.Address, retrievedValidator.Address)
	}
...
func testAttackResistance(t *testing.T, env *TestEnvironment) {
	...
	t.Run("Quality Manipulation", func(t *testing.T) {
		...
		quality := env.pocEngine.qualityAnalyzer.AnalyzeContribution(&suspiciousContrib)
		if quality.OverallScore >= 95.0 {
...
func TestInteroperability(t *testing.T) {
	...
	t.Run("End-to-End Flow", func(t *testing.T) {
		...
		// Store in database
		err := validator.Database.StoreBlock(&Block{
			Height:    0,
			Timestamp: time.Now(),
